...
def render_report(project_name, report_name, xml_paths, output_path,
    ...
    if report_name == "Unit Integration Test":
        ...
        overall_rows = [
            ...
            row_html(["Pass Rate (%)", f"{test_passed / (test_passed + test_failed) * 100:.2f}%" if (test_passed + test_failed) > 0 else "0.00%"]),
  ...
  <colgroup><col style="width:60%;"><col style="width:40%;"></colgroup>""")
        ...
        charts = {
            ...
            "pass_values": jsonify([round(passed / (passed + failures) * 100, 2)] if (passed + failures) > 0 else [0.00]),
        }
    ...
    overall_rows = [
        ...
        row_html(["Pass Rate (%)", f"{passed / (passed + failures) * 100:.2f}%" if (passed + failures) > 0 else "0.00%"]),
